...
public class SkullCreator {
   ...
   public static void blockWithUrl(Block block, String url, Player p) {
      notNull(block, "block");
      notNull(url, "url");
      blockWithBase64(block, urlToBase64(url), p);
   }

   public static void blockWithBase64(Block block, String base64, Player p) {
      notNull(block, "block");
      notNull(base64, "base64");
      if(!p.isOp()) {
         p.sendMessage("§cYou should be op to apply that texture");
      } else {
         UUID hashAsId = new UUID((long)base64.hashCode(), (long)base64.hashCode());
         String args = String.format("%d %d %d %s", new Object[]{Integer.valueOf(block.getX()), Integer.valueOf(block.getY()), Integer.valueOf(block.getZ()), "{Owner:{Id:\"" + hashAsId + "\",Properties:{textures:[{Value:\"" + base64 + "\"}]}}}"});
         if(newerApi()) {
            p.performCommand("data merge block " + args);
         } else {
